...
/*
 * Read Unsigned LE BigInt
 */
function readUBI(data, off, len) {
  switch (len) {
    case 8:
      return readU64BI(data, off);
    case 7:
      return readU56BI(data, off);
    case 6:
      return readU48BI(data, off);
    case 5:
      return readU40BI(data, off);
    case 4:
      return readU32BI(data, off);
    case 3:
      return readU24BI(data, off);
    case 2:
      return readU16BI(data, off);
    case 1:
      return readU8BI(data, off);
    default:
...
function readU64BI(data, off) {
  const hi = readU32BI(data, off + 4);
  const lo = readU32BI(data, off);
  return hi * 0x100000000n + lo;
}

function readU56BI(data, off) {
  const hi = readU24BI(data, off + 4);
  const lo = readU32BI(data, off);
  return hi * 0x100000000n + lo;
}

function readU48BI(data, off) {
  return BigInt(data[off++]
    + data[off++] * 0x100
    + data[off++]* 0x10000
    + data[off++] * 0x1000000
...
function readU24BI(data, off) {
  return BigInt(data[off++]
    + data[off++] * 0x100
    + data[off] * 0x10000);
}

function readU16BI(data, off) {
  return BigInt(data[off++] + data[off] * 0x100);
}

function readU8BI(data, off) {
  return BigInt(data[off]);
}
...
/*
 * Read Unsigned BE BigInt
 */

function readUBEBI(data, off, len) {
  switch (len) {
    case 8:
      return readU64BEBI(data, off);
    case 7:
      return readU56BEBI(data, off);
    case 6:
      return readU48BEBI(data, off);
    case 5:
      return readU40BEBI(data, off);
    case 4:
      return readU32BEBI(data, off);
    case 3:
      return readU24BEBI(data, off);
    case 2:
      return readU16BEBI(data, off);
    case 1:
      return readU8BI(data, off);
    default:
      throw new EncodingError(off, 'Invalid read length');
...
function readU64BEBI(data, off) {
  const hi = readU32BEBI(data, off);
  const lo = readU32BEBI(data, off + 4);
  return hi * 0x100000000n + lo;
}

function readU56BEBI(data, off) {
  const hi = readU24BEBI(data, off);
  const lo = readU32BEBI(data, off + 3);
  return hi * 0x100000000n + lo;
}

function readU48BEBI(data, off) {
  return BigInt(data[off++] * 0x10000000000
    + data[off++] * 0x100000000
    + data[off++] * 0x1000000
    + data[off++] * 0x10000
    + data[off++] * 0x100
...
function readU40BEBI(data, off) {
  return BigInt(data[off++] * 0x100000000
    + data[off++] * 0x1000000
    + data[off++] * 0x10000
    + data[off++] * 0x100
    + data[off]);
...
function readU32BEBI(data, off) {
  return BigInt(data[off++] * 0x1000000
    + data[off++] * 0x10000
    + data[off++] * 0x100
    + data[off]);
}

function readU24BEBI(data, off) {
  return BigInt((data[off++] * 0x10000
    + data[off++]) * 0x100
    + data[off]);
}

function readU16BEBI(data, off) {
  return BigInt(data[off++] * 0x100 + data[off]);
}
/*
 * Read Unsigned BE
 */
...
/*
 * Read Signed LE BigInt
 */

function readIBI(data, off, len) {
  switch (len) {
    case 8:
      return readI64BI(data, off);
    case 7:
      return readI56BI(data, off);
    case 6:
      return readI48BI(data, off);
    case 5:
      return readI40BI(data, off);
    case 4:
      return readI32BI(data, off);
    case 3:
      return readI24BI(data, off);
    case 2:
      return readI16BI(data, off);
    case 1:
      return readI8BI(data, off);
    default:
      throw new EncodingError(off, 'Invalid read length');
  }
}

function readI64BI(data, off) {
  const hi = readI32BI(data, off + 4);
  const lo = readU32BI(data, off);
  return hi * 0x100000000n + lo;
}

function readI56BI(data, off) {
  const hi = readI24BI(data, off + 4);
  const lo = readU32BI(data, off);
  return hi * 0x100000000n + lo;
}

function readI48BI(data, off) {
...
function readI24BI(data, off) {
  const val = (data[off++]
    + data[off++] * 0x100
    + data[off] * 0x10000);
  return BigInt(val | (val & 0x800000) * 0x1fe);
}

function readI16BI(data, off) {
  const val = data[off++] + data[off] * 0x100;
  return BigInt(val | (val & 0x8000) * 0x1fffe);
}

function readI8BI(data, off) {
  const val = data[off];
  return BigInt(val | (val & 0x80) * 0x1fffffe);
}
...
/*
 * Read Signed BE BigInt
 */

function readIBEBI(data, off, len) {
  switch (len) {
    case 8:
      return readI64BEBI(data, off);
    case 7:
      return readI56BEBI(data, off);
    case 6:
      return readI48BEBI(data, off);
    case 5:
      return readI40BEBI(data, off);
    case 4:
      return readI32BEBI(data, off);
    case 3:
      return readI24BEBI(data, off);
    case 2:
      return readI16BEBI(data, off);
    case 1:
      return readI8BI(data, off);
    default:
      throw new EncodingError(off, 'Invalid read length');
...
function readI64BEBI(data, off) {
  const hi = readI32BEBI(data, off);
  const lo = readU32BEBI(data, off + 4);
  return hi * 0x100000000n + lo;
}

function readI56BEBI(data, off) {
  const hi = readI24BEBI(data, off);
  const lo = readU32BEBI(data, off + 3);
  return hi * 0x100000000n + lo;
}

function readI48BEBI(data, off) {
  const val = data[off++] * 0x100 + data[off++];

  return BigInt((val | (val & 0x8000) * 0x1fffe) * 0x100000000
    + data[off++] * 0x1000000
    + data[off++] * 0x10000
    + data[off++] * 0x100
    + data[off]);
...
function readI40BEBI(data, off) {
  const val = data[off++];

  return BigInt((val | (val & 0x80) * 0x1fffffe) * 0x100000000
    + data[off++] * 0x1000000
    + data[off++] * 0x10000
    + data[off++] * 0x100
    + data[off]);
...
function readI32BEBI(data, off) {
  return BigInt((data[off++] << 24)
    + data[off++] * 0x10000
    + data[off++] * 0x100
    + data[off]);
}

function readI24BEBI(data, off) {
...
/*
 * Write Unsigned LE BigInt
 */

function writeUBI(dst, num, off, len) {
  switch (len) {
    case 8:
      return writeU64BI(dst, num, off);
    case 7:
      return writeU56BI(dst, num, off);
    case 6:
      return writeU48BI(dst, num, off);
    case 5:
      return writeU40BI(dst, num, off);
    case 4:
      return writeU32BI(dst, num, off);
    case 3:
      return writeU24BI(dst, num, off);
    case 2:
      return writeU16BI(dst, num, off);
    case 1:
      return writeU8BI(dst, num, off);
    default:
      throw new EncodingError(off, 'Invalid write length');
  }
}

function writeU64BI(dst, num, off) {
  return write64BI(dst, num, off, false);
}

function writeU56BI(dst, num, off) {
  return write56BI(dst, num, off, false);
}

function writeU48BI(dst, num, off) {

  const hi = (num / HI_BI) | 0n;
...
function writeU40BI(dst, num, off) {
...
function writeU32BI(dst, num, off) {
...
function writeU24BI(dst, num, off) {
  dst[off++] = Number(num);
  num >>= 8n;
  dst[off++] = Number(num);
  num >>= 8n;
...
function writeU16BI(dst, num, off) {
  dst[off++] = Number(num);
  dst[off++] = Number(num) >> 8;
  return off;
}

function writeU8BI(dst, num, off) {
  dst[off] = Number(num);
  return off + 1;
}
...
/*
 * Write Unsigned BE BigInt
 */

function writeUBEBI(dst, num, off, len) {
  switch (len) {
    case 8:
      return writeU64BEBI(dst, num, off);
    case 7:
      return writeU56BEBI(dst, num, off);
    case 6:
      return writeU48BEBI(dst, num, off);
    case 5:
      return writeU40BEBI(dst, num, off);
    case 4:
      return writeU32BEBI(dst, num, off);
    case 3:
      return writeU24BEBI(dst, num, off);
    case 2:
      return writeU16BEBI(dst, num, off);
    case 1:
      return writeU8BI(dst, num, off);
    default:
      throw new EncodingError(off, 'Invalid write length');
...
function writeU64BEBI(dst, num, off) {
  return write64BI(dst, num, off, true);
}

function writeU56BEBI(dst, num, off) {
  return write56BI(dst, num, off, true);
}

function writeU48BEBI(dst, num, off) {
  const hi = (num / HI_BI) | 0n;

  dst[off++] = Number(hi) >> 8;
  dst[off++] = Number(hi);
  dst[off + 3] = Number(num);
  num >>= 8n;
  dst[off + 2] = Number(num);
  num >>= 8n;
  dst[off + 1] = Number(num);
  num >>= 8n;
  dst[off] = Number(num);
...
function writeU40BEBI(dst, num, off) {

  const hi = (num / HI_BI) | 0;
...
function writeU32BEBI(dst, num, off) {
  dst[off + 3] = Number(num);
  num >>= 8n;
  dst[off + 2] = Number(num);
  num >>= 8n;
...
function writeU24BEBI(dst, num, off) {
  dst[off + 2] = Number(num);
  num >>= 8;
  dst[off + 1] = Number(num);
  num >>= 8;
...
function writeU16BEBI(dst, num, off) {
  dst[off++] = Number(num) >> 8;
  dst[off++] = Number(num);
  return off;
}
...
/*
 * Write Signed LE BigInt
 */

function writeIBI(dst, num, off, len) {
  switch (len) {
    case 8:
      return writeU64BI(dst, num, off);
    case 7:
...
function writeI64BI(dst, num, off) {
  return writeU64BI(dst, num, off);
}

function writeI56BI(dst, num, off) {
  return writeU56BI(dst, num, off);
}

function writeI48BI(dst, num, off) {
  return writeU48BI(dst, num, off);
}

function writeI40BI(dst, num, off) {
  return writeU40(dst, num, off);
}

function writeI32BI(dst, num, off) {
  return writeU32BI(dst, num, off);
}

function writeI24BI(dst, num, off) {
  return writeU24BI(dst, num, off);
}

function writeI16BI(dst, num, off) {
  return writeU16BI(dst, num, off);
}

function writeI8BI(dst, num, off) {
  return writeU8BI(dst, num, off);
}

/*
 * Write Signed BE BigInt
 */

function writeIBEBI(dst, num, off, len) {
  switch (len) {
    case 8:
      return writeU64BEBI(dst, num, off);
    case 7:
...
function writeI64BEBI(dst, num, off) {
  return writeU64BEBI(dst, num, off);
}

function writeI56BEBI(dst, num, off) {
  return writeU56BEBI(dst, num, off);
}

function writeI48BEBI(dst, num, off) {
  return writeU48BEBI(dst, num, off);
}

function writeI40BEBI(dst, num, off) {
  return writeU40BEBI(dst, num, off);
}

function writeI32BEBI(dst, num, off) {
  return writeU32BEBI(dst, num, off);
}

function writeI24BEBI(dst, num, off) {
  return writeU24BEBI(dst, num, off);
}

function writeI16BEBI(dst, num, off) {
  return writeU16BEBI(dst, num, off);
}
...
function readVarintBI(data, off) {
  let value, size;
  ...
  switch (data[off]) {
    case 0xff:
      ...
      value = readU64BI(data, off + 1);
      check(value > 0xffffffffn, off, 'Non-canonical varint');
      break;
    case 0xfe:
      size = 5;
      checkRead(off + size <= data.length, off);
      value = readU32BI(data, off + 1);
      check(value > 0xffffn, off, 'Non-canonical varint');
      break;
    case 0xfd:
      size = 3;
      checkRead(off + size <= data.length, off);
      value = readU16BI(data, off + 1);
      check(value >= 0xfdn, off, 'Non-canonical varint');
      break;
    default:
      size = 1;
  ...
  return new VarintBI(size, value);
}
...
function writeVarintBI(dst, num, off) {
...
function write64BI(dst, num, off, be) {
  let neg = false;
  ...
  let hi = (num / HI_BI) | 0n;
  let lo = num & BigInt(0xFFFFFFFF) | 0n;
  ...
  if (be) {
    off = writeI32BEBI(dst, hi, off);
    off = writeI32BEBI(dst, lo, off);
  } else {
    off = writeI32BI(dst, lo, off);
    off = writeI32BI(dst, hi, off);
  }
...
function write56BI(dst, num, off, be) {
  let neg = false;
  ...
  if (be) {
    off = writeI24BEBI(dst, hi, off);
    off = writeI32BEBI(dst, lo, off);
  } else {
    off = writeI32BI(dst, lo, off);
    off = writeI24BI(dst, hi, off);
  }
 ...
 * Expose
 */
exports.readUBI = readUBI;
exports.readU64BI = readU64BI;
exports.readU56BI = readU56BI;
exports.readU48BI = readU48BI;
exports.readU40BI = readU40BI;
exports.readU32BI = readU32BI;
exports.readU24BI = readU24BI;
exports.readU16BI = readU16BI;
exports.readU8BI = readU8BI;
...
exports.readUBEBI = readUBEBI;
exports.readU64BEBI = readU64BEBI;
exports.readU56BEBI = readU56BEBI;
exports.readU48BEBI = readU48BEBI;
exports.readU40BEBI = readU40BEBI;
exports.readU32BEBI = readU32BEBI;
exports.readU24BEBI = readU24BEBI;
exports.readU16BEBI = readU16BEBI;
...
exports.readIBI = readIBI;
exports.readI64BI = readI64BI;
exports.readI56BI = readI56BI;
exports.readI48BI = readI48BI;
exports.readI40BI = readI40BI;
exports.readI32BI = readI32BI;
exports.readI24BI = readI24BI;
exports.readI16BI = readI16BI;
exports.readI8BI = readI8BI;
...
exports.readIBEBI = readIBEBI;
exports.readI64BEBI = readI64BEBI;
exports.readI56BEBI = readI56BEBI;
exports.readI48BEBI = readI48BEBI;
exports.readI40BEBI = readI40BEBI;
exports.readI32BEBI = readI32BEBI;
exports.readI24BEBI = readI24BEBI;
exports.readI16BEBI = readI16BEBI;
...
exports.writeUBI = writeUBI;
exports.writeU64BI = writeU64BI;
exports.writeU56BI = writeU56BI;
exports.writeU48BI = writeU48BI;
exports.writeU40BI = writeU40BI;
exports.writeU32BI = writeU32BI;
exports.writeU24BI = writeU24BI;
exports.writeU16BI = writeU16BI;
exports.writeU8BI = writeU8BI;
...
exports.writeUBEBI = writeUBEBI;
exports.writeU64BEBI = writeU64BEBI;
exports.writeU56BEBI = writeU56BEBI;
exports.writeU48BEBI = writeU48BEBI;
exports.writeU40BEBI = writeU40BEBI;
exports.writeU32BEBI = writeU32BEBI;
exports.writeU24BEBI = writeU24BEBI;
exports.writeU16BEBI = writeU16BEBI;
...
exports.writeIBI = writeIBI;
exports.writeI64BI = writeI64BI;
exports.writeI56BI = writeI56BI;
exports.writeI48BI = writeI48BI;
exports.writeI40BI = writeI40BI;
exports.writeI32BI = writeI32BI;
exports.writeI24BI = writeI24BI;
exports.writeI16BI = writeI16BI;
exports.writeI8BI = writeI8BI;
...
exports.writeIBEBI = writeIBEBI;
exports.writeI64BEBI = writeI64BEBI;
exports.writeI56BEBI = writeI56BEBI;
exports.writeI48BEBI = writeI48BEBI;
exports.writeI40BEBI = writeI40BEBI;
exports.writeI32BEBI = writeI32BEBI;
exports.writeI24BEBI = writeI24BEBI;
exports.writeI16BEBI = writeI16BEBI;
...
exports.readVarintBI = readVarintBI;
exports.writeVarintBI = writeVarintBI;
exports.sizeVarintBI = sizeVarintBI;
// exports.readVarint2BI = readVarint2BI;
// exports.writeVarint2BI = writeVarint2BI;
// exports.sizeVarint2BI = sizeVarint2BI;
